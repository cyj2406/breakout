...
const RED_BRICK_COUNT_TARGET = 12;
...
export default function BreakoutGame({ playerName, onQuit }: { playerName: string; onQuit: () => void }) {
    ...
    const updateGame = (time: number) => {
        ...
        if (stateRef.current === 'PLAYING') {
            ...
            timeMsRef.current += dt; setTimeMs(timeMsRef.current);
            ...
            if (b.status === 1) {
                ...
                if (ball.x > bx && ball.x < bx + BRICK_WIDTH && ball.y > by && ball.y < by + BRICK_HEIGHT) {
                    ...
                    if (b.color === COLORS.red) {
                        setRedBricksDestroyed(c => {
                            if (c + 1 >= 3) { syncState('WIN'); (async () => {
                                isSavedRef.current = true;
                                const formatted = formatTime(timeMsRef.current);
                                const name = playerName?.trim() || "Anonymous";
                                try {
                                    await fetch(GOOGLE_SCRIPT_URL, { method: 'POST', mode: 'no-cors', body: JSON.stringify({ name, formattedTime: formatted }) });
                                } catch(e){}
                            })(); stopBGM(); }
                            return c + 1;
                        });
    ...
    const drawGame = (time: number) => {
        ...
        if (stateRef.current !== 'WIN' && stateRef.current !== 'GAMEOVER') {
            ...
            ctx.fillStyle = "white"; ctx.shadowBlur = 8; ctx.shadowColor = "white"; ctx.fill(); ctx.shadowBlur = 0;
        }
        updateGame(time);
    ...
    const handleTouch = (e: React.TouchEvent) => {
        if (!canvasRef.current || stateRef.current === 'PAUSED') return;
    ...
    useEffect(() => {
        ...
        for (let r=0; r<BRICK_ROWS; r++) for (let c=0; c<BRICK_COLS; c++) {
            ...
            if (rc < RED_BRICK_COUNT_TARGET && Math.random() < 0.25) { color = COLORS.red; rc++; }
            nb.push({ col: c, row: r, color, status: 1 });
        }
        bricksRef.current = nb;
        requestRef.current = requestAnimationFrame(drawGame);
        fwRequestRef.current = requestAnimationFrame(updateFireworks);
        (async () => { try { const res = await fetch(GOOGLE_SCRIPT_URL); if (res.ok) setRankings(await res.json()); } catch(e){} })();
    ...
    useEffect(() => {
        if (gameState === 'COUNTDOWN') {
            ...
            if (countdown > 0) setTimeout(() => { setCountdown(c => c - 1); playBeep(220); }, 1000);
            else { syncState('PLAYING'); ballRef.current.dx = 2; ballRef.current.dy = -BALL_SPEED; }
        } else if (gameState === 'RESPAWN_WAIT') {
            setTimeout(() => { syncState('PLAYING'); ballRef.current.dx = 2; ballRef.current.dy = -BALL_SPEED; }, 800);
        }
    }, [gameState, countdown]);
    ...
    return (
        <div className="flex flex-col items-center w-full min-h-screen py-4 md:py-8 touch-none select-none animate-in fade-in duration-700">
            {/* Top Bar */}
            <div className="w-[95%] max-w-[700px] flex justify-between items-center bg-white/5 backdrop-blur-3xl border border-white/10 px-4 py-3 md:px-6 md:py-4 rounded-[1.5rem] md:rounded-[2rem] mb-4 md:mb-6 shadow-2xl">
            ...
            {/* Game Canvas */}
            <div className="relative w-[95%] max-w-[700px] aspect-[7/5] bg-[#0c0c14] rounded-[2rem] md:rounded-[3rem] overflow-hidden border border-white/5 shadow-2xl touch-none"
                ...
                {(gameState === 'GAMEOVER' || gameState === 'WIN') && (
                    <div className="absolute inset-0 flex items-center justify-center bg-black/95 backdrop-blur-3xl z-40 p-4 md:p-6 animate-in fade-in duration-500">
                        <div className="w-full flex flex-col items-center gap-4 text-center">
                            <h2 className={`text-3xl md:text-5xl font-black italic ${gameState === 'WIN' ? 'text-green-400' : 'text-red-500'}`}>{gameState === 'WIN' ? 'SUCCESS' : 'FAILED'}</h2>
                            {gameState === 'WIN' && (
                                <>
                                    <div className="flex flex-col"><span className="text-[9px] text-white/50 uppercase font-bold tracking-widest">Your Record</span><span className="text-4xl md:text-6xl font-black text-white font-mono">{formatTime(timeMs)}</span></div>
                                    <div className="w-full max-w-sm bg-white/5 border border-white/10 rounded-2xl p-4">
                                        <div className="space-y-2">
                                            {rankings.slice(0,3).map((r:any, i) => (
                                                <div key={i} className="flex justify-between items-center px-4 py-2 bg-white/5 rounded-lg border border-white/5 text-sm">
                                                    <span className="font-black text-yellow-400">{i+1}. {r.name || r.name}</span>
                                                    <span className="font-mono font-bold text-white/70">{r.formattedTime || formatTime(r.time)}</span>
                                                </div>
                                            ))}
                                        </div>
                                    </div>
                            ...
                            <button onClick={() => window.location.reload()} className="bg-white text-black px-8 py-3 md:px-12 md:py-4 rounded-xl font-black text-lg hover:scale-105 transition-all w-full max-w-[240px]">RESTART</button>
                        </div>
                    </div>
                )}
            </div>
            ...
            {/* CONTROLS */}
            <div className="flex gap-2 md:gap-4 mt-6 md:mt-8 w-[95%] max-w-[700px]">
                <button onClick={() => syncState(gameState === 'PLAYING' ? 'PAUSED' : gameState === 'PAUSED' ? 'PLAYING' : gameState)} className="flex-[2] py-4 md:py-5 rounded-xl border bg-white/5 border-white/10 text-[10px] md:text-xs font-black text-white/40 uppercase">
                    {gameState === 'PAUSED' ? '▶ RESUME' : '|| PAUSE'}
                </button>
                <button onClick={() => window.location.reload()} className="flex-1 py-4 md:py-5 rounded-xl border bg-white/5 border-white/10 text-[10px] md:text-xs font-black text-white/40 uppercase">↺ RESET</button>
                <button onClick={onQuit} className="flex-1 py-4 md:py-5 rounded-xl border bg-red-500/10 border-red-500/20 text-[10px] md:text-xs font-black text-red-500/40 uppercase">✕ EXIT</button>
            </div>
            <p className="mt-6 md:mt-10 text-[8px] md:text-[10px] text-white/10 font-black uppercase tracking-widest text-center">Design Dept / ID 202301910 / Choi Yu-Jeong</p>
        </div>
    );
}
